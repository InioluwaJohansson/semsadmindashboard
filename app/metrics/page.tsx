...
// Generate random meter readings for demo purposes
const generateRandomMeterReadings = (meterId: string): MeterReading[] => {
  ...
  // Generate data points with varying frequency
  // For recent data (last 24 hours): every 10 seconds
  // For older data: less frequent

  // Last 24 hours: every 10 seconds
  let currentTime = new Date(now.getTime() - 24 * 60 * 60 * 1000) // Start 24 hours ago
  while (currentTime <= now) {
    readings.push(generateReadingForTime(currentTime, meterId))
    currentTime = new Date(currentTime.getTime() + 10 * 1000) // Add 10 seconds
  }

  // Last week: every 5 minutes
  currentTime = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000) // Start 7 days ago
  ...
  currentTime = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000) // Start 30 days ago
  while (currentTime < new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000)) {
    // Until 7 days ago
    readings.push(generateReadingForTime(currentTime, meterId))
    currentTime = new Date(currentTime.getTime() + 60 * 60 * 1000) // Add 1 hour
  }

  // Last 6 months: every day
  currentTime = new Date(startDate)
  while (currentTime < new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000)) {
    // Until 30 days ago
    readings.push(generateReadingForTime(currentTime, meterId))
    currentTime = new Date(currentTime.getTime() + 24 * 60 * 60 * 1000) // Add 1 day
  }

  return readings.sort((a, b) => a.timeValue.getTime() - b.timeValue.getTime())
}

// Helper function to generate a reading for a specific time
const generateReadingForTime = (time: Date, meterId: string): MeterReading => {
  const hour = time.getHours()

  // Vary the values based on time of day
  // Higher consumption during morning and evening hours
  let baseFactor = 1
  if (hour >= 6 && hour < 9)
    baseFactor = 1.5 // Morning peak
  else if (hour >= 17 && hour < 22)
    baseFactor = 1.8 // Evening peak
  else if (hour >= 22 || hour < 6) baseFactor = 0.6 // Night time

  // Add some randomness
  const randomFactor = 0.8 + Math.random() * 0.4

  // Calculate values
  const voltageValue = 220 + Math.random() * 20 // Between 220V and 240V
  const currentValue = baseFactor * randomFactor * (2 + Math.random() * 8)
  const consumptionValue = ((voltageValue * currentValue) / 1000) * (10 / 3600) // kWh for 10 seconds
  const electricityCost = consumptionValue * 0.47 // Cost at ₦0.47 per kWh

  // Add some meter-specific variation
  const meterId_num = Number.parseInt(meterId, 10)
  const meterFactor = ((meterId_num % 4) + 1) * 0.25 + 0.75 // Between 0.75 and 1.75

  return {
    id: Math.floor(Math.random() * 1000000),
    voltageValue: voltageValue * (0.95 + (meterId_num % 10) * 0.01),
    currentValue: currentValue * meterFactor,
    consumptionValue: consumptionValue * meterFactor,
    electricityCost: electricityCost * meterFactor,
    timeValue: time,
  }
}
